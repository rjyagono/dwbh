...
;(function( $, window, document, undefined ){
    ...
    $.fn.accrue.options = {
        ...
        response_basic: 
            '<p style="display:none"><strong>Monthly Payment:</strong><br />P%payment_amount%</p>'+
            '<p><strong>Number of Payments:</strong><br />%num_payments%</p>'+
            '<p><strong>Total Payments:</strong><br />P%total_payments%</p>'+
            '<p style="display:none"><strong>Total Interest:</strong><br />P%total_interest%</p>',
        response_compare: "Save P%savings% in interest!",
	...
	$.loanInfo = function( input ) {
        ...
        var monthly_interest = rate / 100;

        // Now compute the monthly payment amount.
        var interest_amount = ((parseInt(amount)*parseFloat(monthly_interest))*term);
        var monthly = (parseFloat(amount) + parseFloat(interest_amount))/term;
